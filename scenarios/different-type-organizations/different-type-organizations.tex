...
GE Aerospace (\ind{ns1}{ge-aerospace}) operates as both an organization (\cname{iof}{Organization}) and a business organization (\cname{iof}{BusinessOrganization}) with respect to jet engines (\cname{ns1}{jet-engine}). Through its manufacturing division (\ind{ns1}{ge-manufacturer}), which is \opname{bfo}{continuantPartOfAtAllTimes} GE Aerospace, it bears a manufacturer role (\ind{ns1}{jet-engine-production-role}) which it \opname{bfo}{realizes} through a production process (\ind{ns1}{jet-engine-production-process}) that (\opname{iof}{hasOutput}) a jet engine (\ind{ns1}{jet-engine}). The organization also has a business function (\ind{ns1}{jet-engine-selling-function}) which it \opname{bfo}{realizes} through a selling process (\ind{ns1}{jet-engine-selling-process}), where the same jet engines are sold (\opname{bfo}{hasParticipantAtSomeTime}) in the selling process.
