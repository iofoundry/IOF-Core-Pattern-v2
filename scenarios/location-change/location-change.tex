...
An object may be located in different places at different times. In this scenario, the object can be related to multiple locations as \cname{bfo}{Site} by the \opname{bfo}{locatedInAtSomeTime} property. Similarly, the object can be related to multiple instances of \cname{bfo}{TemporalRegion}, each being either a \cname{bfo}{TemporalInterval} or a \cname{bfo}{TemporalInstant} by the \opname{bfo}{existsAt} property. As the object cannot be at two different locations at the same time, the times and the locations need to be paired up to denote at which location the object was at what time. Each pairing can be done by a different \cname{bfo}{SpatioTemporalRegion}. Following the 4D ontology of BFO, the object's spatial and temporal co-occupation can be represented by a \cname{bfo}{SpationTemporalRegion}, which \opname{bfo}{temporallyProjectsOnto} the temporal region the object exists at and \opname{bfo}{spatiallyProjectsOntoAtAllTimes} the \cname{bfo}{SpatialRegion} the object occupies (\opname{bfo}{occupiesSpatialRegionAtAllTimes}). 

Note that the pattern shows that it is the object's location (a \cname{bfo}{Site}) that \opname{}{occupiesSpatialRegionAtAllTimes}. However, an object also occupies the same spatial region that its location occupies. Therefore, this pattern can also be used by directly relating the object to a spatial region by \opname{bfo}{occupiesSpatialRegionAtAllTimes}. The choice depends on the kind of location, which is detailed in \cref{?chapter-space-location}.   

Note further that a \cname{bfo}{SpatioTemporalRegion} cannot project onto two or more different instances of \cname{bfo}{SpatialRegion} while projecting on a single \cname{bfo}{TemporalRegion}, as an object cannot be at two different locations at the same time. But it can be in the same location at different times. Therefore, a \cname{bfo}{SpatioTemporalRegion} can project onto two or more different instances of \cname{bfo}{TemporalRegion} while projecting onto a single \cname{bfo}{SpatialRegion}. However, the data mapping becomes easier if one pair of temporal and spatial regions is related to each spatio-temporal region. If the object is in the same location at different times, two spatio-temporal regions can relate each temporal region to the same spatial region. Observe that these two spatio-temporal regions are simply parts of the spatio-temporal region that projects onto two temporal regions. Therefore, they capture the same semantics while making data mapping easier.     
...
\begin{itemize}
    \item The train \iname{ns1}{freight-train} is located at (\opname{bfo}{locatedInAtSomeTime}) two different locations: \iname{ns1}{powder-river-basin} and \iname{ns1}{long-beach-port}, both of which are instances of \cname{bfo}{site}.
    \item Two separate instances of \cname{bfo}{SpatialRegion}: \iname{ns1}{spatial-region-powder-river-basin} and \iname{ns1}{spatial-region-long-beach}.
    \item The sites are linked to the spatial regions by \opname{bfo}{occupiesSpatialRegionAtAllTimes}.
